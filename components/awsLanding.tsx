...
const STRINGS = {
  en: {
    ...
    lead: "Four skills, nine prompts, and a 37-note knowledge base for agents that write, rewrite, and audit prose with measurable AI-pattern checks.",
    install: "Install skills",
    browse: "Browse knowledge",
    open: "Open knowledge base",
    lim: "Read the limitations",
    metrics: ["4 skills", "9 prompts", "12 levers", "37+ notes", "MIT"],
    trim: "trim",
    ok: "ok",
    ...
    em_redo: "Built on a foundation of rigorous engineering. Delivers value across the stack.",
    em_strike2: "The new release is fast, reliable, and easy — three things that matter.",
    em_redo2: "Fast. Reliable. Easy.",
    em_strike3: "Of course, I'd be happy to help with that — let's dive in.",
    em_redo3: "Tell me what you want done.",
    ...
    examples: [
      {
        ...
        after: "Built on a foundation of rigorous engineering. Uses cutting-edge technologies. Delivers best-in-class performance. Helps every team ship with confidence.",
      },
      {
      ...
      },
      {
        ...
        after: "Three approaches, each with trade-offs. Pick based on your constraints.",
      },
      {
      ...
      },
      {
        ...
        after: "Genium Tasks — CLI for keeping tasks in a git repo. One binary, config in `~/.config/genium/tasks.toml`. 47 tasks across 3 projects on this build, no crashes.",
      },
      {
        pattern: "Closing cliché",
        tag: "P-NEW-7",
        before: "In conclusion, we have thoroughly explored three possible approaches to solving this problem, and each of them undoubtedly has both its advantages and certain limitations. The choice of a specific option depends on the specifics of your project and business requirements. Hopefully, this overview will help you make an informed decision.",
        after: "Three approaches, each with its trade-offs.",
      },
    ],
  },
  ru: {
    kicker: "Скилы для агентской прозы · релиз 2026",
    hero_l1: "Скилы, чтобы текст",
    hero_l2: "меньше походил на модель.",
    lead: "Четыре скила, девять промптов и база из 37+ заметок — для агентов, которые пишут, редактируют и проверяют прозу по измеримым AI-маркерам.",
    install: "Установить",
    browse: "Открыть базу",
    open: "Открыть базу",
    lim: "Честные ограничения",
    metrics: ["4 скила", "9 промптов", "12 рычагов", "37+ заметок", "MIT"],
    trim: "чистка",
    ok: "готово",
    divider: "Tighten → Sufficiency",
    write_less: "Пиши меньше. Доверяй читателю.",
    from_bench: "Заметка редактора",
    bench_body: "Русский формалист Виктор Шкловский показал в 1917 году, что форма — это приём, через который художник делает привычное странным. Хемингуэй, Странк, Уильямс провели тот же урок с американской стороны: доверять читателю, удалять лишнее, останавливаться на повороте.",
    quote: "Если пишущий прозу знает достаточно о том, что он пишет, он может опустить то, что знает, и читатель… ощутит это с той же силой, как если бы автор это сказал.",
    quote_author: "Эрнест Хемингуэй, «Смерть после полудня», 1932",
    pass: "Как работает",
    pass_h: "Три прохода. Одна терминология.",
    pass_sub: "Каждый скил работает в одном цикле. Сначала счёт. Потом переписывание. В конце проверка фактов.",
    pass1_h: "Считаем маркеры.",
    pass1_b: "Считаем AP, D, E, YapScore, вариативность длины предложений, фактичность, плотность форматирования и характеристики голоса.",
    pass1_k: "13 метрик/проход",
    pass2_h: "Применяем 12 рычагов.",
    pass2_b: "STRIP, TIGHTEN, RELY, REBUILD. Проверка сохранности фактов (Lamparth 2026).",
    pass2_k: "12 рычагов",
    pass3_h: "Держим факты.",
    pass3_b: "Сравниваем факты «до» и «после». Если пропало 5–10% имён, дат, чисел, путей — правка отклоняется.",
    pass3_k: "≤10% потерь фактов",
    pass1_s: "Аудит",
    pass2_s: "Переписывание",
    pass3_s: "Проверка",
    research_k: "Основано на исследованиях",
    research_h: "Правила в скилах — не наши.",
    research_b: "Каждый рычаг ведёт к первичному источнику. Мы собираем, переводим, версионируем и публикуем. Писательскую теорию мы не выдумываем.",
    ...
    step: "Шаг за шагом",
    eyebrow_why: "Что делает переписывание",
    eyebrow_ai_tells: "AI-маркеры",
    tells_h: "У AI-текста есть маркеры. Читатель их замечает.",
    tells_b: "Каждая LLM использует один репертуар: em-dashes везде, rule-of-three, hedging, негативные параллелизмы, вежливые поздравления. Читатель это видит, детектор это отмечает. Четыре скила сворачивают этот репертуар обратно в предложения, которые человек писал бы с причиной.",
    ...
    em_redo: "Построено на фундаменте строгой инженерии. Приносит ценность на всём стеке.",
    em_strike2: "Новый релиз быстрый, надёжный и удобный — три вещи, которые важны.",
    em_redo2: "Быстрый. Надёжный. Удобный.",
    em_strike3: "Конечно, я с радостью помогу с этим — давайте разберёмся.",
    em_redo3: "Что нужно сделать?",
    skill_eyebrow: "Четыре скила",
    skill_h: "Редактор, автор, аудитор и часовщик.",
    skill_b: "Покрывают обычный процесс работы без тяжёлого 1000-строчного промпта.",
    install_k: "Установка",
    ...
    prompt_manifest: "manifest.json",
    examples_title: "Что делает переписывание",
    examples_sub: "Шесть паттернов. По одному примеру на каждый.",
    ...
    examples: [
      {
        ...
        after: "Построена на фундаменте строгой инженерии. Использует передовые технологии. Помогает каждой команде поставлять продукт.",
      },
      {
      ...
      },
      {
        ...
        after: "Три подхода, у каждого свои компромиссы. Выбор зависит от ваших ограничений.",
      },
      {
      ...
      },
      {
        ...
        after: "Genium Tasks — CLI для тех, кому надоело вести задачи в Notion. Один бинарь, конфиг в `~/.config/genium/tasks.toml`, задачи живут в git вместе с кодом. 47 задач в трёх проектах на этом билде.",
      },
      {
        ...
        after: "Три подхода, у каждого свои преимущества и ограничения.",
      },
    ],
  },
} as const
...
const SKILLS = [
  {
    ...
    ru: {
      ...
      body: "Пишет README, письма, посты. Считает em-dash, hedging и повторы до того, как пользователь их увидит.",
    },
  },
  {
    icon: "edit",
    ...
    ru: {
      name: "humanize-editor",
      tag: "rewriter · Tighten + Iceberg",
      body: "Переписывает AI-черновик. Сохраняет имена, числа, даты и утверждения — даже после сокращения длины.",
    },
  },
  {
    icon: "loupe",
    ...
    ru: {
      name: "anti-ai-auditor",
      tag: "диагностика · без переписывания",
      body: "Структурированный отчёт: срабатывания паттернов, метрики плотности, приоритеты правок. Сам текст не меняет.",
    },
  },
  {
    icon: "pincers",
...
const SOURCES = [
  { year: 2026, en: ["YapBench", "76 LLMs over-generate 1–3×"], ru: ["YapBench", "76 LLM выдают ответы в 1–3× длиннее нужного"] },
  { year: 2026, en: ["Lamparth", "single-axis mitigation → bias swap"], ru: ["Лампарт", "одностороннее смягчение → подмена bias"] },
  { year: 2026, en: ["MASH (ACL)", "static rewrite, 92% ASR ceiling"], ru: ["MASH (ACL)", "статическая правка, потолок 92% ASR"] },
  { year: 2024, en: ["Binoculars", "zero-shot baseline detector"], ru: ["Binoculars", "zero-shot базовый детектор"] },
  { year: 2024, en: ["Zhang", "format bias from 1% seed"], ru: ["Чжан", "format bias при 1% выборке"] },
  ...
  { year: 1984, en: ["Gasparov", "measure rhythm, not mood"], ru: ["Гаспаров", "измерять ритм, а не настроение"] },
] as const
...
.aws-slider{padding:1.4rem 1.5rem 1.2rem;overflow:hidden}
.aws-slider-head{align-items:flex-start;display:flex;flex-wrap:wrap;gap:.6rem 1rem;justify-content:space-between;margin-bottom:1rem;padding-right:.5rem}
.aws-slider-head .aws-eyebrow{margin-bottom:0;letter-spacing:.14em;padding-left:.05em}
...
.aws-slider-viewport{margin:0;overflow:hidden}
...
@media (max-width:1024px){.aws-hero{gap:1.75rem}.aws-hero h1{font-size:clamp(2.4rem,5.5vw,4rem)}.aws-lead{font-size:1.05rem}.aws-pane .ai-text{font-size:1rem}.aws-pane.before-card,.aws-pane.after-card{padding:1.1rem 1.15rem 1.2rem}.aws-slider{padding:1.2rem 1.25rem 1rem}}
