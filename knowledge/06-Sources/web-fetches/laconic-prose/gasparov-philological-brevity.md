---
type: source
fetched_at: 2026-07-10
url: n/a-bibliographic-source
author: Mikhail Gasparov
year: 1984
source_type: book
applicability: medium
tags: [source, russian-philology, gasparov, brevity, rhythm, lever-12]
status: active
related: [[russian-brevity-grammar], [laconic-prose-models]]
---

# Mikhail Gasparov — Philological brevity and rhythm

## TL;DR / Abstract

Gasparov gives the discipline this project needs: exact terms, tight examples, and respect for rhythm. His work on Russian verse and philological commentary shows how much information a small formal change can carry. For our skills, Gasparov supports measurement: sentence length, rhythm, repetition, and density should be counted before they are aestheticized.

## Key points

No direct quotes here until a checked edition is added. This note anchors 4 claims:

1. Rhythm is measurable.
2. A style claim needs examples, not mood.
3. Philological precision beats broad aesthetic labels.
4. Russian brevity can be described through form, not taste.

## Connection to our work

- **Burstiness:** Gasparov gives permission to count rhythm rather than praise it vaguely.
- **Benchmarking:** sentence length and density metrics belong in `scripts/benchmark-skill.sh`.
- **RU fixtures:** examples should include concrete Russian text, not translated English.
- **Lever 12:** парцелляция and ellipsis need measurable effects.

## Concrete application

Bad audit:

> Текст звучит тяжеловато и немного канцелярски.

Gasparov-informed audit:

> 6 предложений, средняя длина 24 слова, 11 отглагольных существительных, 0 конкретных чисел. Сначала режем синтаксис.

## Open questions

- Add checked references from *Очерк истории русского стиха* (1984).
- Add examples from *Записи и выписки* (2000) for laconic scholarly prose.

## Raw notes

- Михаил Леонович Гаспаров (1935–2005).
- *Очерк истории русского стиха* — 1984.
- *Метр и смысл* — 1999.
- *Записи и выписки* — 2000.
- Relevance: rhythm, concise scholarly sentence, measurement before judgment.
